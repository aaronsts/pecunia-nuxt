...
const { handleSubmit, errors, defineField } = useForm({
	...
	initialValues: {
		...
		transaction_date: new Date(
			transaction.transaction_date.replace(/-/g, "\/").replace(/T.+/, "")
		).toLocaleDateString("sv"), // Using sv here as the Swedish format uses ISO 8601 which is the same for date input
	},
});
...
const updateTransaction = handleSubmit((values) => {
	...
	const data = {
		...
		transaction_date: new Date(
			values.transaction_date.replace(/-/g, "\/").replace(/T.+/, "")
