...
const newTransactionSchema = toTypedSchema(z.object({
  account_id: z.string(),
  payee_id: z.number(),
...
const { handleSubmit, errors, defineField } = useForm({ validationSchema: newTransactionSchema })
  ...
  <div>
    <form @submit="createNewTransaction">
      <div>
        ...
        <input id="description" type="text" v-bind="descriptionProps" v-model="description"
      ...
      <div>
        ...
        <input id="amount" type="number" v-bind="amountProps" v-model="amount"
          class="block w-full rounded-md  border-0 py-1.5 text-neutral-900 shadow-sm ring-1 ring-primary-100 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-primary-300 sm:text-sm sm:leading-6"
          :class="{ '!ring-danger': errors.amount }" />
        <span class="text-danger text-sm">{{ errors.amount }}</span>
      </div>
      <div>
        <label for="account" class="block text-sm font-medium leading-6 text-neutral-900">Account</label>
        <select id="account" v-model="account"
          ...
          <option v-for="(account) in accounts" v-bind="accountProps" :key="account.id" :value="account.id">{{
      account.name }}</option>
        </select>
      ...
      <div>
        ...
        <select id="payee" v-model="payee"
          ...
          <option v-for="(payee) in payees" v-bind="payeeProps" :key="payee.id" :value="payee.id">{{
      payee.name }}</option>
        </select>
      ...
      <div>
        ...
        <select id="category" v-model="category"
          ...
          <option v-for="(category) in categories" v-bind="categoryProps" :key="category.id" :value="category.id">{{
      ...
      <div>
        ...
        <input type="date" name="date" id="date" v-model="date" v-bind="dateProps" />
      </div>
      <button type="submit"
        class="rounded-md mt-4 hover:bg-primary-200 transition-colors px-3 py-2 bg-primary block text-white">Add
        Transaction
      </button>
    </form>
  </div>
