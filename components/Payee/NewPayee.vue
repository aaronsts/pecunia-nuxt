...
const { addPayee } = usePayeeStore()


const newPayeeSchema = toTypedSchema(z.object({
  name: z.string().min(3),
}))

const { handleSubmit, errors, defineField } = useForm({ validationSchema: newPayeeSchema })
...
<template>
  <div>
    <form @submit="createNewPayee" class="sm:col-span-4">
      ...
      <button type="submit"
        class="rounded-md hover:bg-primary-200 transition-colors px-3 py-2 bg-primary block text-white">Add Payee
      </button>
    </form>
  </div>
</template>